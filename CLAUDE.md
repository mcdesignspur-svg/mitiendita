...
- **Claude as operator** — does real web research + supplier outreach and writes results into the *same* tables via the **operator bridge** `lib/operator.ts` (`npm run operator -- report | ingest <file.json> | outreach <supplierId> [product]`). It reads/writes through `lib/db`, so anything ingested shows up in `/admin` immediately.
